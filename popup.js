...
document.addEventListener("DOMContentLoaded", function () {
    ...
    chrome.tabs.executeScript({ code: "var tmpdata = {href : window.location.href,tmpUrl : document.getElementById('tmpUrl') && document.getElementById('tmpUrl').value, url :document.getElementById('url') && document.getElementById('url').value, title : document.getElementsByClassName('live-detail__title')[0].textContent, casts : document.getElementsByClassName('live-detail__cast')[0].textContent };tmpdata", runAt: "document_start" }, function (results) {
        ...
        let title = results && results[0].title.trim().replace(/\s+/g, ' ') || ''; // radio title
        let casts = results && results[0].casts.trim().replace(/\s+/g, ' ').split(/出演者[\s:：]+|ゲスト[\s:：]+/) || ''; // casts
        ...
        chrome.storage.local.get({ "current_recording": false, "timeshift_list": [] }, function (data) {
            ...
            if (tmpUrl.indexOf("https://radiko.jp/v2/api/ts/playlist.m3u8") != -1 && /\/ts\//.test(results[0].href) && !data["timeshift_list"].includes(tmpUrl)) {
                ...
                download_button.onclick = function () {
                    chrome.runtime.sendMessage({ "download-timeshift": tmpUrl, "title": title, "guest": casts[2] }, function () {
                        window.close();
                    });
                }
            } else {
